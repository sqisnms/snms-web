...
import { getCMDuDetail } from "@/actions/cm-actions"
import { CMDuDetailType } from "@/types/cm"
...
export function CmTableDu({ selectedCode, setSelectedCode }: TableProps) {
  const [datas, setDatas] = useState<CMDuDetailType[]>([])
  ...
  const {
    ...
    queryKey: ["getCMDuDetail", selectedCode],
    queryFn: () => getCMDuDetail({ code: selectedCode }),
  ...
  useEffect(() => {
    if (data.length > 0) {
      setColumns([
        { name: "ems_id", comment: "EMS ID", width: "150px" },
        ...
        <TableBody>
          {datas.length === 0 ? (
            ...
            datas.map((d) => (
              ...
              >
                {columns.map((column) => {
                  const value = d[column.name as keyof CMDuDetailType]
